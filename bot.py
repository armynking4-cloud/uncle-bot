...
if bot:
  ...
  def send_welcome(message):
    if message.from_user.id == ADMIN_ID:
      bot.reply_to(
          message,
          "سلام داداش، خودِ ادمینی! ربات با قابلیت چت و عکس‌ساز روشنه 🗿🚬🔥",
      )
    else:
      ...
      bot.reply_to(
          ...
          " /draw رو به همراه توصیف عکس بفرست.",
          reply_markup=markup,
      )
  ...
  def make_image(message):
    ...
    if not prompt:
      bot.reply_to(
          message,
          "لطفاً بعد از دستور /draw توصیف عکسی که می‌خوای رو بنویس!\nمثال:"
          " `/draw a realistic cyberpunk city`",
          parse_mode="Markdown",
      )
      return

    status_msg = bot.reply_to(
        message, "🎨 در حال ساخت عکس با جدیدترین مدل گوگلی... چند لحظه صبر کن!"
    )
    try:
      # ساخت تصویر با جدیدترین مدل Imagen 3
